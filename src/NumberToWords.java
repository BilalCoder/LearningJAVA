...
//        NOTE: When printing words, each word can be in its own line. For example, numberToWords(123); can be: One Two Three
...
public class NumberToWords {
    ...
    public static void numberToWords (int number) {
        ...
        if (reverseDigitCount != numberDigitCount) {
            int differenceOfDigits = numberDigitCount - reverseDigitCount;
            while (differenceOfDigits > 0) {
    ...
    public static int reverse (int number) {
        ...
        int numberCopy = number > 0 ? number : -number;
        while (numberCopy > 0) {
    ...
    public static int getDigitCount (int number) {
        return number >= 0 ? (Integer.toString(number)).length() : -1;
    }
}
